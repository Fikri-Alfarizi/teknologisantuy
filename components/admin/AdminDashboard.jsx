    ...
    <div className="flex w-full">
      ...
      <div className="flex-1 px-8 pb-8 pr-[340px]">
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          ...
          {/* Card 1: Total Suara */}
          <div className="col-span-2 bg-primary-gradient p-8 rounded-3xl text-white shadow-xl shadow-primary/20 relative overflow-hidden group hover:-translate-y-1 transition-transform cursor-pointer">
            ...
            <div className="flex items-start justify-between mb-8 relative z-10">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] opacity-80 mb-2">0 Live</p>
                <h3 className="text-5xl font-extrabold font-headline tracking-tight">{stats.totalVotes}</h3>
                <p className="text-white/80 text-sm mt-2">Total Suara Terkumpul</p>
              </div>
              <RealtimeActiveUsers />
            </div>
            <div className="relative z-10">
              <div className="text-xs uppercase tracking-[0.2em] text-white/80 font-bold">Mendukung Lanjut</div>
            </div>
          </div>

          {/* Card 2: Saran Masuk */}
          <div className="bg-surface-container-lowest p-6 rounded-3xl flex flex-col justify-between shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-6">
              <div className="w-12 h-12 rounded-2xl bg-secondary/10 text-secondary grid place-items-center">
                <span className="material-symbols-outlined">mark_email_read</span>
              </div>
              <span className="text-[10px] uppercase font-bold tracking-[0.25em] text-on-surface-variant">Updated</span>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-on-surface-variant mb-2">Saran Masuk</p>
              <h3 className="text-4xl font-bold font-headline">{stats.feedbackCount}</h3>
            </div>
          </div>
          ...
          <div className="bg-surface-container-lowest p-6 rounded-3xl flex flex-col justify-between shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-6">
              <div className="w-12 h-12 rounded-2xl bg-tertiary-container/10 text-tertiary-container grid place-items-center">
                <span className="material-symbols-outlined">group</span>
              </div>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-on-surface-variant mb-2">User Terdaftar</p>
              <h3 className="text-4xl font-bold font-headline">{totalUsers}</h3>
            </div>
          </div>

          {/* Card 4: Persentase Komunitas */}
          <div className="col-span-4 bg-surface-container-lowest p-8 rounded-3xl shadow-sm">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-on-surface-variant mb-2">Respon Komunitas (SETUJU)</p>
                <h3 className="text-4xl font-extrabold font-headline">{yesPercent}%</h3>
                <p className="text-sm text-on-surface-variant mt-1">Mendukung Lanjut</p>
              </div>
              <div className="grid gap-2 text-sm text-on-surface-variant">
                <span className="font-semibold text-primary">Setuju: {stats.yesVotes}</span>
                <span className="font-semibold">Batal: {stats.noVotes}</span>
              </div>
            </div>
            <div className="mt-6 w-full h-3 bg-surface-container-high rounded-full overflow-hidden">
              <div className="h-full bg-primary-gradient rounded-full transition-all duration-1000" style={{ width: `${yesPercent}%` }} />
            </div>
          </div>
        </div>
            ...
            <table className="min-w-full text-left border-collapse">
              ...
              <tbody className="text-sm font-medium">
                {latestFeedback.length > 0 ? latestFeedback.map((fb) => (
                  ...
                  <tr>
                    <td colSpan={4} className="p-6 text-center text-sm text-on-surface-variant">Tidak ada feedback terbaru.</td>
                  </tr>
                )}
              </tbody>
            </table>
