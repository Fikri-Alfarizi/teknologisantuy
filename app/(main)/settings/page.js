...
export default function ProfileSettings() {
  ...
  if (!user && !session) {
    return (
      <div className="flex items-center justify-center min-h-[70vh] p-4 font-sans bg-[#2b2d31]">
        <div className="bg-[#1e1f22] p-8 border-4 border-black text-center box-shadow-solid max-w-md w-full text-white">
          <FaUserCircle className="text-6xl text-yellow-500 mx-auto mb-4" />
          <h2 className="text-2xl font-black uppercase mb-2 text-yellow-500">Akses Ditolak</h2>
          <p className="font-bold text-gray-400 mb-6 text-sm">Harap login terlebih dahulu untuk mengakses pengaturan profil.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="settings-wrapper">
      <div className="settings-container">
        <h1 className="page-title"><FaUserCircle className="title-icon" /> Pengaturan Profil</h1>
             ...
             <div className="flex flex-col gap-4">
                ...
                <div className="linked-card border-discord">
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <div className="text-sm font-black uppercase text-white">Akun Discord</div>
                   ...
                   {!session ? (
                     <button onClick={() => nextAuthSignIn('discord')} className="link-btn text-discord border-discord hover:bg-discord hover:text-white">Sambungkan</button>
                   ) : (
                ...
                <div className="linked-card border-google">
                   <div className="flex items-center gap-4">
                     ...
                     <div>
                       <div className="text-sm font-black uppercase text-white">Akun Google</div>
                       <div className="text-xs text-gray-400 font-bold">
                   ...
                   {!user?.providerData?.find(p => p.providerId === 'google.com') ? (
                     <button onClick={linkGoogle} className="link-btn text-google border-google hover:bg-google hover:text-white">Sambungkan</button>
                   ) : (
                     <span className="badge-connected">AKTIF</span>
                   )}
                </div>
      ...
      <style jsx>{`
        ...
        .linked-card { display: flex; align-items: center; justify-content: space-between; background: #2b2d31; border: 2px solid #000; padding: 16px; border-radius: 4px; flex-wrap: wrap; gap: 12px; }
        ...
        .link-btn { background: transparent; border: 2px solid; padding: 6px 12px; font-weight: 900; font-size: 10px; text-transform: uppercase; cursor: pointer; transition: 0.2s; }
